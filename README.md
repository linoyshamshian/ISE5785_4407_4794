...
## שלב מיני־פרויקט 2 – יצירת תמונת נוף הררי עם ירח

בשלב הבא של הפרויקט, בחרנו ליצור סצנה חדשה – תמונת נוף לילית עם הרים וירח.  
מקור ההשראה לתמונה נלקח מתוך תמונה שראינו ב־Pinterest, ששילבה הרים חדים עם ירח בולט ומרשים.
זו התמונה המקורית:

<img src="final_images/Moon_Texture.png" width="320" alt="תמונת ההשראה" />


השתמשנו בקוד שפיתחנו במיני־פרויקט 1, שבונה סצנה המבוססת על משולשים המופקים מתמונה, אך הפעם החלטנו להוסיף עוד מורכבות ובחרנו להוסיף ירח.

למרות שהוספת ירח לסצנה נשמעת פשוטה – לא הסתפקנו ביצירת כדור לבן פשוט.  
מטרתנו הייתה לדמות את הירח בצורה ריאליסטית ככל האפשר, כולל עומק וטקסטורה.

לשם כך, השתמשנו בטקסטורת תמונה של ירח אמיתי, ובנינו את הירח באמצעות סידרה של כדורים קטנים (spheres), שכל אחד מהם נצבע לפי ערך הפיקסל המתאים בתמונה.

כך יצרנו אובייקט תלת־ממדי שמורכב מאלפי נקודות, המעניקות תחושת טקסטורה אמיתית – במקום משטח חלק עם אפקטים מלאכותיים.

### רעיון הקוד

הקוד סורק את תמונת הטקסטורה של הירח בשני צעדים:

- בלולאה חיצונית לפי קואורדינטת Y (גובה)
- בלולאה פנימית לפי קואורדינטת X (רוחב)

עבור כל נקודה בתמונה:
- מחושב מיקום תלת־ממדי על פני כדור (באמצעות המרות זוויתיות: θ ו־φ)
- אם הצבע בתמונה כהה מאוד (שחור), מדלגים על הנקודה – כדי לשמור על מבנה הירח בלבד
- אם הפיקסל מואר, נוצרת Sphere (כדור קטן) עם צבע תואם ונוספת לסצנה

התוצאה – ירח בעל מראה ריאליסטי, עם עומק ונפח, שנראה עשוי מנקודות אור עדינות ומרקם אמיתי.

כל הגאומטריה של הירח נשמרת כחלק מהסצנה ב־Java.

כדי לבדוק ולהריץ את הירח בנפרד, יצרנו טסט ייעודי שהתמקד ביצירת הירח בלבד – ללא שאר מרכיבי הסצנה.  
מטרת הטסט הייתה לאפשר ניסוי וטעייה בתהליך מיפוי הטקסטורה על גבי כדור, עד להשגת תוצאה משכנעת.
...
<img src="final_images/Moon_Texture.png" width="320" alt="נוף הררי עם ירח" />

---

ניתן למצוא את כל התמונות הסופיות בתיקיית <code>final images</code>, עם שמות קבצים תיאוריים עבור כל סצנה.
